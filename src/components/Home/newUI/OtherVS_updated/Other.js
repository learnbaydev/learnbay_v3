...
const Other = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (headDivRef.current) {
        ...
        const maxScroll =
          headDivRef.current.scrollWidth - headDivRef.current.clientWidth;
  ...
  const scrollToSection = (index) => {
    if (headDivRef.current) {
      const containerWidth = headDivRef.current.clientWidth;
      const scrollAmount =
        index === 0 ? 0 : headDivRef.current.scrollWidth - containerWidth; // Scroll to end for dot 1
        ...
        <div className={styles.left}>
          ...
          {otherData.leftSide.map((item, index) => (
            <div key={index}>
              ...
              {index < otherData.leftSide.length - 1 && <hr />}
            </div>
          ))}
        </div>
          ...
          {otherData.middleSide.map((content, index) => (
            <div key={index}>
              <div className={styles.middleContent}>
                <Image
                  src="https://d32and0ii3b8oy.cloudfront.net/adlearnbay/check_circle.webp"
                  width={25}
                  height={25}
                  alt="icon"
                  priority
