...
// import whyChooseData from "./WhylbdatalimitedISIO"; // Import the data

// function AnimationNew3({ fullstack }) {
...
//                       <p>{item.certification.text}</p>
//                     </div>
//                     <div>
//                       <Image
//                         src={item.certification.imgSrc}
//                         alt="Microsoft Logo"
//                         width={150} // replace with actual dimensions or make dynamic
//                         height={50}
//                         loading="lazy"
...
// export default AnimationNew3;

import React, { useEffect, useRef } from "react";
import styles from "./whyChooseSection.module.css";
import Image from "next/image";
import whyChooseData from "./WhylbdatalimitedISIO"; // Ensure path is correct
...
function AnimationNew3({ fullstack }) {
  ...
  const getInsideBoxClassName = (sectionIndex, insideIndex) => {
    ...
    if (sectionIndex === 1) return styles.insideBoxDesign2;
              ...
              <div className={styles.svglogo}>
                ...
                {index <= whyChooseData.length - 2 && (
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                    height={index === 1 ? 260 : index === 2 ? 200 : 320}
                    viewBox="0 0 24 500"
                    fill="none"
                  >
                    <path
                ...
                {index === 2 ? (
                  <div className={styles.customSection}>
                    ...
                    <div>
                      <Image
                        src={item.certification.imgSrc}
                        alt={item.certification.imgAlt || "Logo"}
                        width={150}
                        height={50}
                        loading="lazy"
                        style={{ marginLeft: "16px" }}
                  ...
                  >
                    {item.subItems.map((subItem, subIndex) => (
                      <div
                        ...
                        key={subIndex}
                      >
                        {/* THE FIX: Only render Image if subItem.icon exists */}
                        {subItem.icon && (
                          <Image
                            src={subItem.icon}
                            alt="icon"
                            width={subItem.width || 50}
                            height={subItem.height || 50}
                            loading="lazy"
                          />
                        )}
                        <p>{subItem.text}</p>
